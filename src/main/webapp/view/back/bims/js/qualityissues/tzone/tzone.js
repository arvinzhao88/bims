...
//路径配置
tzone.url=ctx+"/bims/tzone/";
tzone.requestUrl = {
		queryList:tzone.url+"getList"
};
...
tzone.filed =[
	...
	},
	{
		...
		"createdCell": function (nTd, sData, oData, iRow, iCol) {
			var html="----";
			if(oData.processStatus == "1" && oData.isRft == "1" && oData.subSource == "1") {
	...
	},
	{
		...
		"createdCell": function (nTd, sData, oData, iRow, iCol) {
			...
			if(oData.processStatus == 1) {
				if (oData.subSource ==2 || (oData.subSource==1 && oData.isRft == 1 && oData.processStatus == 1)){//web 提交
					html += '<a href="#" onclick="del('+sData+')" data-target=".this-true"><i class="fa fa-trash"></i> 删除</a></h4><br>' +
						'<a href="#" onclick="toEdit('+sData+')" data-target=".this-true"><i class="fa fa-edit"></i> 修正</a></h4>';
					$(nTd).html(html);
				} else if (oData.subSource ==1 && oData.isRft == 0 && oData.processStatus == 1) {//pad 提交
					html += '<a href="#" onclick="del('+sData+')" data-target=".this-true"><i class="fa fa-trash"></i> 删除</a></h4><br>' +
						'<a href="#" onclick="toSend('+sData+')" data-target=".this-true"><i class="fa fa-edit"></i> 查看</a></h4>';
					$(nTd).html(html);
				}else {
					html += '<a href="#" onclick="del('+sData+')" data-target=".this-true"><i class="fa fa-trash"></i> 删除</a></h4><br>' +
						'<a href="#" onclick="toEdit('+sData+')" data-target=".this-true"><i class="fa fa-edit"></i> 修正</a></h4>';
			...
			}else if(oData.processStatus == 2){
				html +=	'<a href="#" data-toggle="modal" data-target=".this-time" onclick="getDealLog('+sData+')"><i class="fa fa-info-circle"></i>详情</a></h4><br>'+
					'<a href="#" onclick="toBill('+sData+')" data-target=".this-true"><i class="fa fa-edit"></i> 查看</a></h4>';
				$(nTd).html(html);
			}else if(oData.processStatus == 3){
				html += '<a href="#" data-toggle="modal" data-target=".this-time" onclick="getDealLog('+sData+')"><i class="fa fa-info-circle"></i>详情</a></h4><br>'+
					'<a href="#" onclick="toTool('+sData+')" data-target=".this-true"><i class="fa fa-edit"></i> 查看</a></h4>';
				$(nTd).html(html);
			}
			else if(oData.processStatus == 4){
				//bims/apucheck/apuEqr
				html += '<a href="#" data-toggle="modal" data-target=".this-time" onclick="getDealLog('+sData+')"><i class="fa fa-info-circle"></i>详情</a></h4><br>'+
					'<a href="#" onclick="toTool('+sData+')" data-target=".this-true"><i class="fa fa-edit"></i> 查看</a></h4>';
				$(nTd).html(html);
			}else if (oData.processStatus == 8) {
				html += '处理中';
				$(nTd).html(html);
			}*/
			...
			var html = "----";
			// T区已发送处理意见
			if (oData.isRft == '0'  && oData.processStatus == '8') {
				html = '<a href="#" onclick="send('+ sData +')" data-target=".this-true"><i class="fa fa-edit"></i>查看</a>';
			}
			// T区不合格，跳发送处理意见页面
			if (oData.isRft == '0' && oData.processStatus == '1') {
				html = '<a href="#" onclick="send('+ sData +')" data-target=".this-true"><i class="fa fa-edit"></i>处理</a>';
			}
			// 接收后，跳修正页面
			if(oData.qualityType == 'T' && oData.isRft == '1' && oData.processStatus == "1" && oData.subSource == "1") {
				html = '<a href="#" onclick="edit(' + sData + ')" data-target=".this-true"><i class="fa fa-edit">修正</i></a>';
			}
			// 质量工程师提交，直接处理
			if(oData.qualityType == 'T' && oData.processStatus == "1" && oData.subSource != "1") {
				html = '<a href="#" onclick="isEqr(' + sData + ')" data-target=".this-true"><i class="fa fa-edit">处理</i></a>';
			}
			// 已修正，跳页面
			if (oData.processStatus > '1' && oData.processStatus != '7' && oData.processStatus != '8') {
				html = '<a href="#" onclick="isEqr('+ sData +')" data-target=".this-true"><i class="fa fa-edit"></i>处理</a>';
			}
			// 已关闭
			if (oData.processStatus == '7') {
				html = '<a href="#" onclick="isEqr('+ sData +')" data-target=".this-true"><i class="fa fa-edit"></i>查看</a>';
			}
			$(nTd).html(html);
		}
	}
		...
		ajax: function (data, callback, settings) {
			...
			$.ajax({
				...
				data: {
					...
					"isEqr": $("#isEqr option:selected").val(), // 是否eqr
...
//去选择单据页面
function toBill(id){
	window.location.href=ctx+'/bims/common/Eqr?id='+id;
}

//去选择工具页面
function toTool(id){
	window.location.href=ctx+'/bims/common/Learder?id='+id;
}
