...
//路径配置
sqa.url=ctx+"/bims/sqa/";
sqa.requestUrl = {
		queryList:sqa.url+"getList"
};
...
sqa.filed =[
	...
	{
		"data": "processMethod",	//处理方式
		"createdCell": function (nTd, sData, oData, iRow, iCol) {
			var html = "";
			if (oData.dealToolPerson != null && oData.dealToolPerson != "") {
				for ( var i = 0; i < oData.dealToolPerson.length; i++){
					html += oData.dealToolPerson[i].toolName +'/'+oData.dealToolPerson[i].processPersonName+'<br>';
				}
				$(nTd).html(html);
			}else {
				html += "---/---";
	...
	},
	{
		...
		"createdCell": function (nTd, sData, oData, iRow, iCol) {
			...
			if(oData.processStatus == "1") {
				html = '<a href="#" onclick="toSqaEdit(' + sData + ')" data-target=".this-true"><i class="fa fa-edit">修正</i></a>';
			}
			// 已修正或质量工程师自己提交，跳eqr页面
			if ((oData.processStatus > '1' && oData.processStatus != '7') || oData.processStatus == 2 && oData.subSource != "1") {
				html = '<a href="#" onclick="isSqaEqr('+ sData +')" data-target=".this-true"><i class="fa fa-edit"></i>处理</a>';
			}
			// 已关闭
			if (oData.processStatus == '7') {
				html = '<a href="#" onclick="isSqaEqr('+ sData +')" data-target=".this-true"><i class="fa fa-edit"></i>查看</a>';
			}
			$(nTd).html(html);
