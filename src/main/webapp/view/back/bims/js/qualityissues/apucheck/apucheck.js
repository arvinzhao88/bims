...
//路径配置
apucheck.url=ctx+"/bims/apucheck/";
apucheck.requestUrl = {
...
apucheck.filed =[
	...
	},
	{
		...
		"createdCell": function (nTd, sData, oData, iRow, iCol) {
			/*var html ="";
			if(oData.processStatus == 1) {	//已提交(待处理)
				...
				$(nTd).html(html);
			}else if(oData.processStatus == 2){	//已修正(待处理)
				...
				$(nTd).html(html);
			}else if(oData.processStatus == 3){	//已选单据(待处理)
				if(oData.isEqr == 1) {	//是EQR处理
					html += '<a href="#" data-toggle="modal" data-target=".this-time" onclick="getDealLog('+sData+')"><i class="fa fa-info-circle"></i>详情</a></h4><br>'+
			...
			else if(oData.processStatus == 4){	//已选处理工具(待审核)
				html += '<a href="#" data-toggle="modal" data-target=".this-time" onclick="getDealLog('+sData+')"><i class="fa fa-info-circle"></i>详情</a></h4><br>'+
					'<a href="#" onclick="toTool('+sData+')" data-target=".this-true"><i class="fa fa-edit"></i> 查看</a></h4>';
				$(nTd).html(html);
			}
			else if(oData.processStatus == 7){	//已关闭
				html += '关闭';
				$(nTd).html(html);
			}
			else {
			...
			// 已修正或质量工程师自己提交，跳eqr页面
			if ((oData.processStatus > '1' && oData.processStatus != '7') || oData.processStatus == 1 && oData.subSource != "1") {
