...
//路径配置
eqa.url=ctx+"/bims/eqa/";
eqa.requestUrl = {
		queryList:eqa.url+"getList"
};
	...
	},
	{
		...
		"createdCell": function (nTd, sData, oData, iRow, iCol) {
			...
			if ((oData.processStatus > '1' && oData.processStatus != '7') || oData.processStatus == 1 && oData.subSource != "1") {
				html = '<a href="#" onclick="isEqaEqr('+ sData +')" data-target=".this-true"><i class="fa fa-edit"></i>处理</a>';
			}
			// 已关闭
			if (oData.processStatus == '7') {
				html = '<a href="#" onclick="isEqaEqr('+ sData +')" data-target=".this-true"><i class="fa fa-edit"></i>查看</a>';
			}
			$(nTd).html(html);
		}
	}
...
//处理
function isEqaEqr(id){
	window.location.href=ctx+'/bims/eqa/eqaEqr?id='+id;
}

//更新
function toEdit(id){
	window.location.href=ctx+'/bims/eqa/eqaCorrect?id='+id;
}
