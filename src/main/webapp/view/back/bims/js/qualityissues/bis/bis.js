...
//路径配置
bis.url=ctx+"/bims/bis/";
bis.requestUrl = {
		queryList:bis.url+"getList"
};
	...
	},
	{
		...
		"createdCell": function (nTd, sData, oData, iRow, iCol) {
			...
			if ((oData.processStatus > '1' && oData.processStatus != '7') || oData.processStatus == 1 && oData.subSource != "1") {
				html = '<a href="#" onclick="toBisEqr('+ sData +')" data-target=".this-true"><i class="fa fa-edit"></i>处理</a>';
			}
			// 已关闭
			if (oData.processStatus == '7') {
				html = '<a href="#" onclick="toBisEqr('+ sData +')" data-target=".this-true"><i class="fa fa-edit"></i>查看</a>';
			}
			$(nTd).html(html);
		}
	}
...
//更新
function toBisEqr(id){
	window.location.href=ctx+'/bims/bis/bisEqr?id='+id;
}

//更新
function toEdit(id){
	window.location.href=ctx+'/bims/bis/bisCorrect?id='+id;
}
