...
//路径配置
area.url=ctx+"/bims/desk/";
area.requestUrl = {
		queryList:area.url+"areatolist"
};
...
area.filed =[
	...
	{
		"data": "issueCategoryL1",	//质量分类
		"createdCell": function (nTd, sData, oData, iRow, iCol) {
            var html = "----";
			if (oData.qualityType == "A" || oData.qualityType == "M") {
                if(oData.issueCategoryL1 == "1") {
                    html = '自制件';
                }
                if(oData.issueCategoryL1 == "2") {
                    html = '采购件';
                }
                if(oData.issueCategoryL1 == "3") {
                    html = '在制发动机';
                }
			}
			if (oData.qualityType == "T") {
				html = oData.l1name;
				$(nTd).html(html);
			}
			$(nTd).html(html);
	...
	},
	{
		...
		"createdCell": function (nTd, sData, oData, iRow, iCol) {
			var html ="";
			if (oData.processStatus =="0"){
				html += '<a href="#" onclick="receive('+sData+',\'' + oData.qualityType + '\')" data-target=".this-true"><i class="fa fa-get-pocket"></i> 接收</a></h4><br>'
				$(nTd).html(html);
			}else{
				html +="已接收";
				$(nTd).html(html);
...
//接收
function receive(id,area) {
	if (area == 'A' || area == 'T' || area == 'M') {
		window.location.href = ctx + '/bims/apucheck/apuIssueDesc?id=' + id;
	}
	if (area == 'B' || area == 'E' || area == 'S') {
		layer.confirm('确定接收问题吗？', {icon: 3, title:'系统提示'}, function(index){
			$.ajax({
				...
				success : function(result) {
					if(result.success) {
						layer.msg('已接收');
						window.location.href = ctx+'/back/index';
					}
