                                ...
                                <form id="docManage" data-parsley-validate class="form-horizontal form-label-left">
                                    ...
                                    <table class="doc" id="doc" width="100%" border="0" cellspacing="0" cellpadding="0"
                                        ...
                                        <tr id="tempMeasures">
                                            ...
                                            <td style="width:18%;"><select id="measures" name="tempMeasures" class="form-control" required>
                                                <option value="">--请选择--</option>
                                                <option value="1" <c:if test="${measures == 1}">selected="selected"</c:if>>料废</option>
                                                <option value="2" <c:if test="${measures == 2}">selected="selected"</c:if>>工废</option>
                                                <option value="3" <c:if test="${measures == 3}">selected="selected"</c:if>>让步接收</option>
                                                <option value="4" <c:if test="${measures == 4}">selected="selected"</c:if>>待处理</option>
                                                <option value="6" <c:if test="${measures == 6}">selected="selected"</c:if>>筛选</option>
                                                <option value="7" <c:if test="${measures == 7}">selected="selected"</c:if>>拒收</option>
                                                <option value="8" <c:if test="${measures == 8}">selected="selected"</c:if>>返工</option>
                                            </select></td>
                                    ...
                                    <div class="form-group" id="isOk">
                                        <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                                            <button class="btn btn-primary" type="button" <%--data-toggle="modal"
                                                    data-target=".this-go"--%> onclick="confirm(${issue.id})">确认
                                            </button>
                                            <button class="btn btn-default" type="button" onClick="toApuCorrect(${issue.id})">返回
                                            </button>
                                        </div>
                                    </div>
                                </form>
                        ...
                        <c:if test="${fn:contains(person.belongRole, '4') == true && issue.isEqr=='1'}">
                            <div class="x_content" id="eqrQDiv" style="display: block;">
                                <form id="demo-formEqr" data-parsley-validate class="form-horizontal form-label-left">
                                    ...
                                    <table width="100%" border="0" cellspacing="0" cellpadding="0"
                                        ...
                                        <tr>
                                            ...
                                            <td>${issueInfo.plantname}/${issueInfo.areaname}<br>
                                                <input type="hidden" name="issueArea" value="${issueInfo.plantname}/${issueInfo.areaname}">
                                            </td>
                                        ...
                                        <tr>
                                            ...
                                            <td colspan="4" rowspan="3"><textarea name="containmentAction"
                                                                                  ...
                                                                                  rows="3"
                                                                                  style="width:100%" required>${notification.containmentAction}</textarea>
                                            </td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            <td><strong>Effect Engine Amout<br>
                                                影响发动机台数</strong></td>
                                            <td><input type="text" id="effectEngineAmout" name="effectEngineAmout" onblur="check(this)" onkeyup="this.value=this.value.replace(/[^0-9.]/g,'')"
                                                       required="required" class="form-control" value="${notification.effectEngineAmout}" placeholder="请输入数字"></td>
                                        ...
                                        <tr>
                                            ...
                                            <td><input type="text" id="effectToDelivery" name="effectToDelivery"
                                                       required="required" class="form-control" value="${notification.effectToDelivery}"></td>
                                        </tr>
                                        <tr>
                                    ...
                                    <h2>邮件接收人</h2>
                                    <select id="sendEmail" name="email" class="form-control" multiple="multiple" required>
                                    </select>
                                    <!--按钮-->
...
<script src="${resPath}/view/back/bims/js/qualityissues/sqa/dealTools.js"></script>
<script type="text/javascript">
    /**左侧菜单操作**/
...
<script type="text/javascript">
    ...
    function closeIssue(issueId) {
        /*layer.confirm('确定关闭问题吗？', {icon: 3, title: '系统提示',offset: "50%"}, function (index) {*/
            //do something
        if ($("#form_close").valid()) {
            $('#modal_close').modal('hide');
            $.post("${webPath}/bims/apucheck/closeIssue", {
                "id": issueId,
...
<script>
  ...
  //加载已经保存的单据input
            ...
            layer.confirm('确定将该问题交由EQR Leader进行处理吗？', {icon: 3, title: '系统提示'}, function (index) {
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        }else if (qualityType == 'J') {
                            /**子菜单*/
                            window.location.href = '${webPath}/back/toJobOne';
                        }else if (qualityType == 'B') {
                            /**子菜单*/
                            window.location.href = '${webPath}/back/toBis';
                        }else if (qualityType == 'E') {
                            /**子菜单*/
                            window.location.href = '${webPath}/back/toEqa';
                        }else if (qualityType == 'S') {
                            /**子菜单*/
                            window.location.href = '${webPath}/back/toSqa';
                        }else if (qualityType == 'H') {
                            /**子菜单*/
                            window.location.href = '${webPath}/aftersale/toAfterSale';
                        }
                    }
                })
                layer.close(index);
    ...
    function getTagsById(id, type) {
        if (type == 1) {
            window.open('${webPath}/Tagstprocessed/jumpPendingForm/' + id, '_blank', 'width=1200,height=800,top=0,left=0,toolbar=no,menubar=no,scrollbars=yes, resizable=yes,location=no, status=no')
        }
        if (type == 2) {
            window.open('${webPath}/ScrapBill/queryScrapForUpdate/' + id, '_blank', 'width=1200,height=800,top=0,left=0,toolbar=no,menubar=no,scrollbars=yes, resizable=yes,location=no, status=no')
        }
        if (type == 3) {
            layer.msg("暂无数据")
            return;
        ...
        if (type == 4) {
            window.open('${webPath}/PendingDocument/queryPendingDocuments/' + id, '_blank', 'width=1200,height=800,top=100px,top=0,left=0,toolbar=no,menubar=no,scrollbars=yes, resizable=yes,location=no, status=no')
        }
        if (type == 5) {
            layer.msg("暂无数据")
            return;
        ...
        if (type == 6) {
            window.open('${webPath}/NonProductContainment/queryNonconformingProductContainmentOrder/' + id, '_blank', 'width=1200,height=800,top=0,left=0,toolbar=no,menubar=no,scrollbars=yes, resizable=yes,location=no, status=no')
        }
        if (type == 7) {
            layer.msg("暂无数据")
            return;
